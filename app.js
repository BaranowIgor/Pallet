...
boxForm.addEventListener("submit", function (event) {
  ...
  function calculateMaxBoxes() {
    ...
    if (maxLoadWeight < boxWeight) {
      ...
      document.getElementById('volumeOccupied').style.display = 'none';
      return;
    }
  ...
  function maxBoxesResult() {
    try {
      ...
      const volumeInfo = calculateMaxBoxes();
      ...
      const volumeOccupied = (((volumeInfo.boxVolume * totalMaxBoxes) / volumeInfo.palletVolume)*100).toFixed(1);
      ...
      if (totalMaxBoxes <= 0) {
        ...
        volumeDescription = `Volume Occupied: <strong>${volumeOccupied}%</strong>`;
        ...
        console.log(volumeInfo.boxVolume, maxBoxesByWeight, volumeInfo.palletVolume);
        const volumeOccupied = (((volumeInfo.boxVolume * forEmptyPalletInfo.maxBoxesByWeight) / volumeInfo.palletVolume)*100).toFixed(1);
        volumeDescription = `Volume Occupied: <strong>${volumeOccupied}%</strong>`;
    ...
    document.getElementById('volumeOccupied').innerHTML = volumeDescription;
